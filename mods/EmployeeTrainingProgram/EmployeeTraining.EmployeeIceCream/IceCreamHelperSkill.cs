...
public class IceCreamHelperSkill : EmployeeSkill<IceCreamHelperSkill, IceCreamHelperSkillTier, EmplIceCreamHelper, IceCreamHelper>
{
	...
	private static readonly IceCreamHelperSkillTier[] SKILL_TABLE = BuildTable();

	private static IceCreamHelperSkillTier[] BuildTable()
	{
		int[] exp =
		{
		...
		float dstMax = 5f;
		IceCreamHelperSkillTier[] table = new IceCreamHelperSkillTier[100];
		for (int i = 0; i < 100; i++)
		{
			float t = (rapiditySrc[i] - srcMin) / (srcMax - srcMin);
			table[i] = new IceCreamHelperSkillTier
			{
				Lvl = i + 1,
				Exp = exp[i],
	...
	public override IceCreamHelper Employee
	{
		get => base.Employee;
		set => base.Employee = value;
	}

	public override string JobName => "Ice Cream Helper";
	...
	public float ActionInterval => 0.48f / ((float)base.Tier.Dexterity / 100f);

	public float ProductPlacingIntv => 0.32f / ((float)base.Tier.Dexterity / 100f);

	public float ServesPerMinute => 60f / Mathf.Max(ActionInterval, 0.01f);

	internal override IceCreamHelperSkillTier[] SkillTable => SKILL_TABLE;
	...
	public IceCreamHelperSkill(IceCreamHelperSkillData data)
		: base(data)
	{
	}
	...
	{
		Singleton<IDManager>.Instance.IceCreamHelperSO(base.Id).DailyWage = dailyWage;
		Singleton<IDManager>.Instance.IceCreamHelperSO(base.Id).HiringCost = hiringCost;
	}

	public override void Setup()
	{
		base.InitialWage = Singleton<IDManager>.Instance.IceCreamHelperSO(base.Id).DailyWage;
		base.InitialHiringCost = Singleton<IDManager>.Instance.IceCreamHelperSO(base.Id).HiringCost;
		UpdateStatus(init: true);
		base.OnLevelChanged = (Action<bool>)Delegate.Combine(base.OnLevelChanged, (Action<bool>)delegate
		{
			IceCreamHelperLogic.ApplyRapidity(Employee);
		});
	}
